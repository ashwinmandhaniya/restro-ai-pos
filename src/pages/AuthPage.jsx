...
const AuthPage = () => {
  ...
  const handleGoogleCredential = useCallback(async (response) => {
    ...
    try {
      ...
      if (status === 403 || status === 404 || errorCode === 'USER_NOT_FOUND' || errorCode === 'NO_ACTIVE_PLAN' || errorCode === 'PENDING_APPROVAL') {
        navigate(`/user-not-found?email=${encodeURIComponent(userEmail)}`);
        return;
      }
    } finally {
        ...
        >
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <p style={{ textAlign: 'center', fontSize: 13, color: '#777', margin: 0 }}>
              {isLogin ? "New to RestroxAI? " : "Already have an account? "}
              <button
                type="button"
                onClick={switchMode}
                style={{
